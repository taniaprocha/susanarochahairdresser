...
export default class Quotes extends React.PureComponent {
  ...
  handlePrevious = () => {
    ...
    this.setState(
      state => ({
        first: false,
        forward: false,
        index: state.index === 0 ? this.quotes.length - 1 : state.index - 1,
      }),
      () => {
        this.setTimeout()
      }
  ...
  get quotes() {
    return this.props.shuffle ? this.shuffledQuotes : this.props.quotes
  }
  ...
  setTimeout(animation = true) {
    if (!this.isAnimating) {
      ...
      this.timeout = window.setTimeout(
        this.handleForward,
        animation ? this.delay : this.shortDelay
      )
    }
  }
